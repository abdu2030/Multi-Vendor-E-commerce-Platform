  ...
  return (
    <main className="min-h-screen bg-stone-50 text-stone-900">
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-14">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-base leading-relaxed text-stone-500">
              Every public product here is approved, attached to an active
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <form
          className="grid gap-3 rounded-3xl border border-stone-200 bg-white p-4 shadow-sm lg:grid-cols-[minmax(0,1.3fr)_220px_180px_140px_140px_auto]"
          action="/products"
        >
          ...
          <select
            ...
            defaultValue={getString(params.categorySlug)}
            name="categorySlug"
          >
          ...
          <button
            className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-5 text-sm font-extrabold text-white transition hover:bg-emerald-700 lg:col-start-6"
            type="submit"
          >
            Search
        ...
        <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-widest text-emerald-600">
              {products.meta.total} approved listing
              {products.meta.total === 1 ? "" : "s"}
            </p>
            <h2 className="mt-1 text-2xl font-extrabold text-stone-950">
              Product catalog
            </h2>
          </div>
          <Link
            className="text-sm font-extrabold text-stone-500 transition hover:text-stone-900"
            href="/products"
          >
            Reset filters
          </Link>
        ...
        {loadError ? (
          <div className="mt-6 rounded-3xl border border-red-200 bg-red-50 p-5 text-sm font-semibold text-red-700">
            Products could not load. Start the API server on port 5000 and
            refresh this page.
          </div>
        ) : null}
        ...
        {products.items.length ? (
          ...
          <div className="mt-6 rounded-3xl border border-stone-200 bg-white p-10 text-center shadow-sm">
            ...
            <h3 className="mt-4 text-lg font-extrabold text-stone-950">
              No approved products found
            </h3>
            <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-stone-500">
              Try clearing filters, or approve a seller product from the admin
              workflow before testing public visibility.
            </p>
          </div>
        )}
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex items-center justify-between text-xs font-bold text-stone-400">
          ...
          <span>
            {product.counts.variants} variant
            {product.counts.variants === 1 ? "" : "s"}
          </span>
        </div>
      </div>
    </Link>
...
function Pagination({
  ...
  return (
    <div className="mt-8 flex items-center justify-between rounded-3xl border border-stone-200 bg-white p-3 shadow-sm">
      <Link
        className={`rounded-2xl px-4 py-2 text-sm font-extrabold ${page <= 1 ? "pointer-events-none text-stone-300" : "text-stone-700 hover:bg-stone-100"}`}
        href={withPage(params, page - 1)}
      >
        Previous
      </Link>
      ...
      <Link
        className={`rounded-2xl px-4 py-2 text-sm font-extrabold ${page >= totalPages ? "pointer-events-none text-stone-300" : "text-stone-700 hover:bg-stone-100"}`}
        href={withPage(params, page + 1)}
      >
        Next
      </Link>
...
const filterClassName =
  "h-12 rounded-2xl border border-stone-200 bg-stone-50 px-4 text-sm font-semibold text-stone-800 outline-none transition focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/10";
